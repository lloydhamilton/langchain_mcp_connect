...
class TestStdioServerParameters:
  ...
  def test_server_mapping(self) -> None:
    ...
    assert mcpServers.mcpServers["stdioserver"] == StdioServerParameters(command="uvx", args=["./"])
    assert mcpServers.mcpServers["sseserver"] == SseServerParameters(url="http://localhost:8080/sse")
